...
import { ArrowBigUp, MessageSquare, Ellipsis, Reply } from 'lucide-react';
...
const UserPost = ({ width, isPreview = true, isComment = false }: PostProps) => {
  return (
   <div className={`${width} border rounded-lg border-zinc-200 dark:border-zinc-800 flex flex-col items-center px-1 pb-1 bg-inherit bg-inherit`}>
    <div className="pt-2 px-3 w-full flex items-center space-x-2">
     ...
     <div className={`px-3 w-full flex flex-col ${isPreview ? "text-ellipsis overflow-hidden" : ""}`}>
      ...
      <p className={`${isPreview ? "line-clamp-3" : ""} dark:text-zinc-400 text-sm md:text-base`}>
