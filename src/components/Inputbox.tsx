...
const Inputbox = ({ placeholder, icon, type, name, isTransparent, value, onKeyDown, onChange, ref, toggleType }: INPUTBOX_TYPE) => {
  ...
  const attributes: Partial<React.InputHTMLAttributes<HTMLInputElement> & React.TextareaHTMLAttributes<HTMLTextAreaElement>> = {
    className: `${isTransparent ? `border-none outline-none bg-transparent py-2 text-zinc-700 dark:text-zinc-200 flex-grow` : `dark:bg-zinc-900 bg-zinc-50 border border-zinc-300 text-zinc-700 focus:ring-1 border-zinc-300 dark:border-zinc-800 focus:ring-emerald-500 focus:border-none dark:focus:ring-emerald-600 outline-none focus:border-none w-full p-2 rounded-md ${icon ? "pl-11" : ""} mb-0 dark:text-zinc-200`}`, 
    name: name ?? "", 
